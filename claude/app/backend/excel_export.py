...
def build_excel(result: dict[str, Any]) -> bytes:
    try:
        return _build_excel(result)
    except Exception as exc:
        return _build_fallback_excel(result, exc)


def _build_fallback_excel(result: dict[str, Any], exc: Exception) -> bytes:
    """Минимальный XLSX, чтобы пользователь получил диагностируемый файл вместо HTTP 500."""
    buf = BytesIO()
    params = result.get("params", {}) or {}
    with pd.ExcelWriter(buf, engine="xlsxwriter") as writer:
        pd.DataFrame([
            ("Статус", "Экспорт сформирован в аварийном режиме"),
            ("Причина", str(exc)),
            ("Дата формирования", datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
            ("Период", _period_text(params.get("period") or {})),
        ], columns=["Параметр", "Значение"]).to_excel(writer, sheet_name="Параметры выборки", index=False)
        pd.DataFrame(result.get("summary") or []).to_excel(writer, sheet_name="Сводная", index=False)
        pd.DataFrame({"Аналитическая подсказка": result.get("analytics") or ["Экспорт построен без расширенного оформления."]}).to_excel(
            writer, sheet_name="Аналитические подсказки", index=False
        )
        for ws in writer.sheets.values():
            ws.set_zoom(105)
            ws.set_column(0, 0, 34)
            ws.set_column(1, 20, 22)
    return buf.getvalue()


def _build_excel(result: dict[str, Any]) -> bytes:
    params = result.get("params", {}) or {}
